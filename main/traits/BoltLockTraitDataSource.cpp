...
void BoltLockTraitDataSource::InitiateLock(int32_t aLockActor)
{
	...
	else
	{
		...
		NRF_LOG_INFO("ProcessTraotChanges COmplete");
...
void BoltLockTraitDataSource::InitiateUnlock(int32_t aLockActor)
{
	...
	else
	{
		...
		// Start an app timer for 2 seconds to simulate time taken
		// by mechanical motor to move the deadbolt.
	}
}

void BoltLockTraitDataSource::LockingSuccessful(void)
{
	NRF_LOG_INFO("Lock State is now LOCKED!");
...
{
	NRF_LOG_INFO("Lock State is now UNLOCKED!");
